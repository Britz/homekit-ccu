...
describe('HomeKit-CCU Tests ' + testCase, () => {
  ...
  it('HomeKit-CCU check slat LEVEL 0%', (done) => {
    that.server._ccu.fireEvent('HmIP.7515755417ABCD:4.LEVEL_2', 0)
    let accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    let service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
  ...
  it('HomeKit-CCU check slat LEVEL 50%', (done) => {
    that.server._ccu.fireEvent('HmIP.7515755417ABCD:4.LEVEL_2', 0.5)
    let accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    let service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    let chCur = service.getCharacteristic(Characteristic.CurrentHorizontalTiltAngle)
  ...
  it('HomeKit-CCU check slat LEVEL 100%', (done) => {
    that.server._ccu.fireEvent('HmIP.7515755417ABCD:4.LEVEL_2', 1)
    let accessory = that.server._publishedAccessories[Object.keys(that.server._publishedAccessories)[0]]
    let service = accessory.getService(Service.WindowCovering)
    assert.ok(service, 'WindowCovering not found')
    let chCur = service.getCharacteristic(Characteristic.CurrentHorizontalTiltAngle)
  ...
  it('HAP-Homematic set LEVEL 50%', (done) => {
    ...
    chTar.setValue(50, () => {
      setTimeout(async () => {
        let value = await that.server._ccu.getValue('HmIP.7515755417ABCD:4.LEVEL')
        try {
  ...
  it('HAP-Homematic set slats 0%', (done) => {
    ...
    chTar.setValue(-90, () => {
      setTimeout(async () => {
        let value = await that.server._ccu.getValue('HmIP.7515755417ABCD:4.LEVEL_2')
        try {
